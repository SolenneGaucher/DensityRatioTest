...
for (chosen_cell in cell_choice){
  ...
  res_cell <- foreach (id = levels(data_chosen_cell$unique_patient), .combine=rbind) %dopar%{
    print(paste("We begin the experiment for id ", id, ", time is ", now()))
    source("utils/aux.R")
    source("utils/test_MMD.R")
    library(lubridate)
    
    data_test <- data_chosen_cell[data_chosen_cell$unique_patient == id,]
    if (nrow(data_test) > max_size_test){
      data_test <- data_test[sample(nrow(data_test), max_size_test), ]
    }
    ...
    if (power[1] > 0){
      ...
      best_alpha <- alpha[max(which(power == 0))]
    }
    print(paste("Best alpha found for id ", id, ", time is ", now()))
    ...
    print(paste("cell type is :", chosen_cell, ", index is :", id, ", time is now :", now()))
    print("")
    print("")
