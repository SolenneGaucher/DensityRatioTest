...
for (setting in c("A", "B", "C")){
  for (n_train in n_train_choice){
    res_exp <- foreach(i = 1:n_sim, .combine=rbind) %dopar% {
      ...
      # Compute the threshold for the bootstrap DRT
      threshold <- quantile_dr_bootstrap(data_est, partition, parms_est, B_iterations, alpha)
      ...
      for (theta in theta_choice){
        ...
        res <- rbind(res, c("BRDT", setting, n_train, theta, 
                            test_dr_bootstrap(data_est, data_test, partition, parms_est, threshold)))
      }
      names(res) <- c("method" , "setting", "n_train", "theta", "result")
      return(res)
    }
    print(paste("setting = ", setting,", n_train = ", n_train, ", time is ", now()))
    saveRDS(res_exp, paste("results/Detection_rate_BDRT/setting_",setting, "_ntrain", n_train,".rds", sep = ""))
  }
}
